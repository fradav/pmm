...
function(_pmm_ensure_vcpkg dir rev)
    ...
    list(GET st 1 msg)
    if (rc)
        _pmm_log(FATAL_ERROR "Failed to download vcpkg [${rc}]: ${msg}")
    endif ()
    ...
    if (NOT len EQUAL 1)
        _pmm_log(FATAL_ERROR "More than one directory extracted from downloaded vcpkg [??]")
    endif ()
    ...
    if (_PMM_RC)
        _pmm_log(FATAL_ERROR "Failed to Bootstrap the vcpkg tool [${_PMM_RC}]:\n${_PMM_OUTPUT}")
    endif ()
    _pmm_log("Testing bootstrapped vcpkg")
    _pmm_exec("${vcpkg_root}/vcpkg${CMAKE_EXECUTABLE_SUFFIX}" version)
    if (_PMM_RC)
        _pmm_log(FATAL_ERROR "Failed to execute generated vcpkg tool [${_PMM_RC}]:\n:${_PMM_OUTPUT}")
    endif ()
    ...
    elseif (plat STREQUAL "arm64")
        ...
        elseif (gen MATCHES "^Visual Studio 1. 20..$")
            ...
            elseif (CMAKE_SIZEOF_VOID_P EQUAL 8)
                ...
                _pmm_log(FATAL_ERROR "Unable to determine target triple for vcpkg.")
            endif ()
        endif ()
    endif ()
...
function(_pmm_vcpkg_copy_custom_ports ports_list)
    foreach (port IN LISTS ports_list)
        ...
        if (NOT EXISTS "${port}/portfile.cmake")
            _pmm_log(FATAL_ERROR "Failed find portfile in ${port}!")
        endif ()
        ...
        if (EXISTS "${port_location}" AND NOT EXISTS "${port_location}/CUSTOM_PORT_FROM_PMM.txt")
            _pmm_log(WARNING "Portfile already included by default!")
...
function(_pmm_vcpkg)
    ...
    if (NOT DEFINED ARG_REVISION)
        ...
        _pmm_log(FATAL_ERROR "Using pmm(VCPKG) requires a REVISION argument. Try `REVISION 2019.07`")
    endif ()
    ...
    if (ARG_REQUIRES)
        ...
        if (_PMM_RC)
            _pmm_log(FATAL_ERROR "Failed to install requirements with vcpkg [${_PMM_RC}]:\n${_PMM_OUTPUT}")
        else ()
