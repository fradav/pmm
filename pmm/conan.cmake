set(PMM_CONAN_MIN_VERSION 1.18.0 CACHE INTERNAL "Minimum Conan version we support")
set(PMM_CONAN_MAX_VERSION 1.19.99 CACHE INTERNAL "Maximum Conan version we support")
...
function(_pmm_ensure_conan)
    ...
    if (NOT py3_exe AND NOT py2_exe)
        _pmm_log(FATAL_ERROR "No conan executable found, and no Python was found to install it.")
    endif ()
endfunction()
...
function(_pmm_conan_uninstall)
    ...
    if (NOT EXISTS "${_PMM_CONAN_VENV_DIR}")
        _pmm_log(FATAL_ERROR "Conan executable '${PMM_CONAN_EXECUTABLE}' was not installed by PMM. We will not uninstall it.")
    endif ()
...
function(_pmm_conan_upgrade)
    ...
    if (_PMM_RC)
        _pmm_log(FATAL_ERROR "Conan upgrade failed [${_PMM_RC}]")
    endif ()
...
function(_pmm_conan_get_settings out)
    ...
    if (NOT "CXX" IN_LIST langs)
        ...
        if (NOT "C" IN_LIST langs)
            _pmm_log(FATAL_ERROR "pmm(CONAN) requires that either C or C++ languages be enabled.")
        endif ()
    endif ()
    ...
    if (comp_id STREQUAL GNU)
        ...
        if (NOT comp_version MATCHES "${majmin_ver_re}")
            _pmm_log(FATAL_ERROR "Unable to parse compiler version string: ${comp_version}")
        endif ()
    ...
    elseif (comp_id STREQUAL "MSVC")
        ...
        _pmm_log(FATAL_ERROR "Unable to detect compiler setting for Conan from CMake. (Unhandled compiler ID ${comp_id}).")
    endif ()
...
function(_pmm_conan_run_install _build_type _generator_name)
    ...
    if (NOT EXISTS "${__conan_inc}")
        _pmm_log(DEBUG "Need to run conan install: Conan include file not found")
        set(do_install TRUE)
    elseif (NOT EXISTS "${prev_cmd_file}")
        _pmm_log(DEBUG "Need to run conan install: Never been run")
        set(do_install TRUE)
    elseif ("${conanfile}" IS_NEWER_THAN "${prev_cmd_file}")
    ...
    if (NOT do_install)
        ...
        if (_PMM_RC)
            _pmm_log(FATAL_ERROR "Conan install failed [${_PMM_RC}]:\n${_PMM_OUTPUT}")
        else ()
...
function(_conan_ensure_remotes remotes)
    ...
    foreach (line IN LISTS lines)
        if (NOT line MATCHES "^([^:]+): (.*) \\[Verify SSL: (.+)\]")
            _pmm_log(WARNING "Unparseable `conan remote list` line: ${line}")
        else ()
    ...
    while (TRUE)
        ...
        if (len EQUAL 1)
            _pmm_log(FATAL_ERROR "REMOTES list of pmm(CONAN) must be pairs of name and value")
        elseif (len EQUAL 0)
            break()
        endif ()
        ...
        if (NOT head MATCHES "(.+);(.+)")
            _pmm_log(FATAL_ERROR "Bad remote arguments? ${head}")
        endif ()
        set(name "${CMAKE_MATCH_1}")
        set(url "${CMAKE_MATCH_2}")
        set(verify_ssl True)
        ...
        if (NOT (url IN_LIST all_urls))
            ...
            if (_PMM_RC)
                _pmm_log(FATAL_ERROR "Failed to add Conan remote ${name} [${_PMM_RC}]: ${_PMM_OUTPUT}")
            endif ()
        endif ()
    endwhile ()
endfunction()
...
function(_pmm_conan)
    ...
    if (NOT CONAN_PREV_EXE STREQUAL PMM_CONAN_EXECUTABLE)
        ...
        if (_PMM_RC)
            ...
            unset(PMM_CONAN_EXECUTABLE CACHE)
            _pmm_log(FATAL_ERROR "Conan executable (${exe}) seems invalid [${_PMM_RC}]:\n${_PMM_OUTPUT}")
        endif ()
    ...
    if (NOT DEFINED conanfile AND NOT (CONAN_EXPORTED AND CONAN_IN_LOCAL_CACHE))
        _pmm_log(FATAL_ERROR "pf(CONAN) requires a Conanfile in your project source directory")
    endif ()
...
function(_pmm_script_main_conan)
    ...
    if (ARG_/Rebuild OR ARG_/UpdatePackages)
        ...
        if (NOT prev_cmds)
            _pmm_log(FATAL_ERROR "PMM hasn't been run before")
            return()
        endif ()
        ...
        foreach (cmd_file ${prev_cmds})
            ...
            message_hr()
            set(cmd "${CMAKE_COMMAND}" -E env CLICOLOR_FORCE=1 ${cmd})
    ...
    if (ARG_/Create AND ARG_/Export)
        _pmm_log(FATAL_ERROR "/Export and /Create can not be specified together")
    endif ()

    if (ARG_/Create)
        if (NOT ARG_/Ref)
            _pmm_log(FATAL_ERROR "Pass a /Ref for /Create")
        endif ()
        _pmm_ensure_conan()
        execute_process(
        ...
        if (retc)
            _pmm_log(FATAL_ERROR "Create failed [${retc}]")
        endif ()
    endif ()
    if (ARG_/Export)
        if (NOT ARG_/Ref)
            _pmm_log(FATAL_ERROR "Pass /Ref when for /Export")
        endif ()
        _pmm_ensure_conan()
        execute_process(
        ...
        if (retc)
            _pmm_log(FATAL_ERROR "Export failed [${retc}]")
        endif ()
    endif ()
    ...
    if (ARG_/Upload)
        ...
        if (ARG_/Ref MATCHES ".+@.+")
            ...
            if (_PMM_RC)
                _pmm_log(FATAL_ERROR "Failed to get package info [${_PMM_RC}]:\n${_PMM_OUTPUT}")
            endif ()
            if (NOT _PMM_OUTPUT MATCHES "([^\n]+)@PROJECT.*")
                _pmm_log(FATAL_ERROR "Can't parse Conan output [${_PMM_RC}]:\n${_PMM_OUTPUT}")
            endif ()
        ...
        if (retc)
            _pmm_log(FATAL_ERROR "Upload failed [${retc}]")
        endif ()
    endif ()
endfunction()
